...
import { Star, Copy, Check, Sparkles, Users, ExternalLink } from 'lucide-react';
...
export const GoogleReviewsSection = ({ darkMode }: GoogleReviewsSectionProps) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'google_reviews' }, () => loadData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'google_review_settings' }, () => loadData())
      .subscribe();
  ...
  const handleReviewClick = () => {
    if (settings?.review_link) {
      window.open(settings.review_link, '_blank');
    }
  };
  ...
  const handleCopyLink = () => {
    if (settings?.review_link) {
      ...
      setCopied(true);
      toast({ title: 'Link copiado!', description: 'Envie para seus clientes avaliarem' });
  ...
  if (reviews.length === 0) {
    return (
      <section className="bg-card rounded-2xl shadow-xl p-8 mb-6">
        <div className="text-center text-muted-foreground">Nenhuma avaliação cadastrada ainda</div>
      </section>
    );
  }
    ...
    <section className="relative bg-gradient-to-br from-blue-950 via-blue-900 to-blue-950 rounded-2xl shadow-2xl p-8 sm:p-10 mb-6 border-4 border-yellow-500 overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl sm:text-5xl font-black mb-2 text-white drop-shadow-lg">
          ...
          <p className="text-white/80 text-sm mb-4">
            R. Gomes Freire, 434 - Lapa, São Paulo - SP
          </p>
          ...
          {settings?.review_link && (
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
              ...
              <Button
                ...
                size="lg"
                variant="outline"
                className="bg-white/10 hover:bg-white/20 text-white border-2 border-yellow-400 font-bold px-6 py-6 text-base backdrop-blur-sm transition-all duration-300 hover:scale-105"
              >
                {copied ? (
                  <>
